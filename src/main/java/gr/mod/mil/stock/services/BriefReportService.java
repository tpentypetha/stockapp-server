...
public class BriefReportService {
    ...
    public List<BriefReport> getBriefReport(Date from, Date to){
       ...
       String sql="SELECT D.NAME,SUM(T.WITHDRAWAMOUNT), SUM(C.PRICE) , SUM(C.NUMBEROFPAGES) " +
               "    FROM Consumable_Transaction T, CONSUMABLE C, DEPARTMENTS D " +
               "    WHERE T.DEPTID!=0 AND T.CONSUMABLE_ID=C.ID AND T.WITHDRAWAMOUNT>0 AND T.DEPTID=D.ID " +
