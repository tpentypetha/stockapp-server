...
public class DepReportService {
    ...
    public List<ExtendListReport> getDepReport(Date from, Date to,long depid){

        String sql="SELECT DISTINCT D.NAME, C.CODE, (SELECT SUM(T2.WITHDRAWAMOUNT) " +
                "                                FROM Consumable_Transaction T2 " +
                "                                WHERE T.CONSUMABLE_ID=T2.CONSUMABLE_ID " +
                "                                AND T.DEPTID=T2.DEPTID) AS Withdrawls, " +
                "                                (SELECT c2.NUMBEROFPAGES " +
                "                                FROM CONSUMABLE c2 " +
                "                                WHERE c2.ID=C.ID) as pages " +
                "FROM Consumable_Transaction T,DEPARTMENTS D,CONSUMABLE C " +
                "WHERE T.DEPTID=D.ID AND T.CONSUMABLE_ID=C.ID " +
                "AND T.WHEN >= ? AND T.WHEN <= ? AND T.DEPTID=? " +
                "ORDER BY D.NAME ";
